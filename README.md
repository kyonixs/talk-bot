...
- **配信スケジュール:**
  ...
  | レポート | 実行タイミング |
  |---|---|
  | 米国株 日次 | NY時間の平日 市場閉場30分後 JST（夏05:30 / 冬06:30）※曜日・祝日はNY基準で判定 |
  | 日本株 日次 | JST平日 15:30 JST |
  | 米国株 週次 | 土曜 15:00 JST |
  | 日本株 週次 | 土曜 14:00 JST |
...
└── services/
    ...
    ├── trending_service.py     # トレンドデータ取得 (Google Trends + はてブ)
...
2. **Google Sheets** から銘柄リストを取得（`{market}_Holdings` / `{market}_Watchlist` シート、リトライ付き）
